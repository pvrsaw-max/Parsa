...
export const fresh=()=>({schemaVersion:16,players:[],scores:{},turn:0,round:1,roundLimit:3,phase:"setup",winner:null,usedQuestions:[],usedChallenges:[],usedDuels:[],usedPowers:{},coupUsed:{},stats:{},history:[],cat:null,value:null,question:null,activePower:null,powerTarget:null,auction:null,auctionStage:null,auctionDeadline:null,optionsRevealed:false,resolving:false,tiePlayers:[],powerDraft:{index:0,hands:{},choices:{}},pendingHandoff:false});
...
export const validate=s=>{const e=[];if(!s||typeof s!=="object")return["state"];if(!Array.isArray(s.players))return["players"];if(new Set(s.players).size!==s.players.length)e.push("duplicate-players");if(!["setup","secret","board","values","question","tiebreak","finished"].includes(s.phase))e.push("phase");if(s.players.length&&(s.turn<0||s.turn>=s.players.length))e.push("turn");if(!Number.isInteger(s.roundLimit)||s.roundLimit<1||s.roundLimit>12)e.push("round-limit");if(!Number.isInteger(s.round)||s.round<1||s.round>s.roundLimit+1)e.push("round");for(const n of s.players){if(typeof s.scores?.[n]!=="number"||!Number.isFinite(s.scores[n]))e.push("score:"+n);if(!s.stats?.[n])e.push("stats:"+n);if(!Array.isArray(s.usedPowers?.[n]))e.push("powers:"+n)}return e};
export function setRoundLimit(s,rounds){const n=Number(rounds);if(s.phase!=="setup"||!Number.isInteger(n)||n<1||n>12)return false;s.roundLimit=n;return true}
...
export function commitTurn(s){if(!s.players.length||s.phase!=="question"||!s.resolving)return false;s.activePower=null;s.powerTarget=null;s.auction=null;s.auctionStage=null;s.auctionDeadline=null;s.resolving=false;s.question=null;s.cat=null;s.value=null;s.optionsRevealed=false;s.turn++;if(s.turn>=s.players.length){s.turn=0;s.round++}if(s.round>s.roundLimit)finishOrTie(s);else s.phase="board";return true}
...
export function canCoup(s,n){if(!n||!s.players.includes(n)||s.round!==s.roundLimit||s.coupUsed[n])return false;const lead=Math.max(...s.players.map(x=>s.scores[x]));return lead>0?s.scores[n]<lead*.6:s.scores[n]<lead-400}
...
export function hydrate(raw){const d=fresh(),s={...d,...(raw&&typeof raw==="object"?raw:{})};s.players=Array.isArray(s.players)?s.players.filter((n,i,a)=>typeof n==="string"&&n.trim()&&!["__proto__","prototype","constructor"].includes(n)&&a.indexOf(n)===i):[];s.turn=Number.isInteger(s.turn)&&s.turn>=0&&s.turn<Math.max(1,s.players.length)?s.turn:0;s.roundLimit=Number.isInteger(s.roundLimit)&&s.roundLimit>=1&&s.roundLimit<=12?s.roundLimit:3;s.round=Number.isInteger(s.round)&&s.round>=1&&s.round<=s.roundLimit+1?s.round:1;if(!["setup","secret","board","values","question","tiebreak","finished"].includes(s.phase))s.phase="setup";s.scores=s.scores&&typeof s.scores==="object"?s.scores:{};s.stats=s.stats&&typeof s.stats==="object"?s.stats:{};s.usedPowers=s.usedPowers&&typeof s.usedPowers==="object"?s.usedPowers:{};s.coupUsed=s.coupUsed&&typeof s.coupUsed==="object"?s.coupUsed:{};s.history=Array.isArray(s.history)?s.history:[];s.usedQuestions=Array.isArray(s.usedQuestions)?s.usedQuestions:[];s.usedChallenges=Array.isArray(s.usedChallenges)?s.usedChallenges:[];s.usedDuels=Array.isArray(s.usedDuels)?s.usedDuels:[];s.tiePlayers=Array.isArray(s.tiePlayers)?s.tiePlayers:[];s.powerDraft=s.powerDraft&&typeof s.powerDraft==="object"?s.powerDraft:d.powerDraft;s.powerDraft.hands=s.powerDraft.hands&&typeof s.powerDraft.hands==="object"?s.powerDraft.hands:{};s.powerDraft.choices=s.powerDraft.choices&&typeof s.powerDraft.choices==="object"?s.powerDraft.choices:{};s.powerDraft.index=Number.isInteger(s.powerDraft.index)?s.powerDraft.index:0;for(const n of s.players){if(typeof s.scores[n]!=="number"||!Number.isFinite(s.scores[n]))s.scores[n]=0;const st=s.stats[n];if(!st||typeof st!=="object")s.stats[n]={correct:0,wrong:0,duels:0};else{s.stats[n]={correct:Number.isInteger(st.correct)&&st.correct>=0?st.correct:0,wrong:Number.isInteger(st.wrong)&&st.wrong>=0?st.wrong:0,duels:Number.isInteger(st.duels)&&st.duels>=0?st.duels:0}}if(!Array.isArray(s.usedPowers[n]))s.usedPowers[n]=[];else s.usedPowers[n]=[...new Set(s.usedPowers[n].filter(x=>PERSONAL_POWERS.includes(x)))];if(typeof s.coupUsed[n]!=="boolean")s.coupUsed[n]=false}s.optionsRevealed=!!s.optionsRevealed;s.resolving=!!s.resolving;s.pendingHandoff=!!s.pendingHandoff;s.schemaVersion=16;return s}
