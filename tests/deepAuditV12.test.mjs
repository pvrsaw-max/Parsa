...
// V23 bank invariants: player-facing bank has 550 unique questions plus 60 challenges.
...
const playerKeys=['هنر و معماری','علم و طبیعت','ریاضی و منطق','ورزش','فیلم و سریال','موسیقی','اطلاعات عمومی','تاریخ','جغرافیا','غذا و خوراکی','حیوانات','فناوری و اینترنت','ایران و فرهنگ','ادبیات و زبان','بازی و سرگرمی','خودرو و حمل‌ونقل','اختراعات و اکتشافات'];
const playerAll=playerKeys.flatMap(cat=>Object.values(bank[cat]||{}).flat());
T('550 player questions',playerAll.length===550);
T('550 unique player ids',new Set(playerAll.map(q=>q.id)).size===550);
T('550 unique player text',new Set(playerAll.map(q=>q.text.trim())).size===550);
T('all four unique options',all.every(q=>q.opts.length===4&&new Set(q.opts).size===4));
T('valid correct index',all.every(q=>Number.isInteger(q.correctIndex)&&q.correctIndex>=0&&q.correctIndex<4));
T('player categories',playerKeys.every(k=>bank[k]));
T('legacy aliases removed from public bank',!Object.keys(bank).some(k=>k.includes('🎨')||k.includes('🔬')||k.includes('⚽')||k.includes('🎭')));
T('new categories present',['تاریخ','جغرافیا','غذا و خوراکی','حیوانات','فناوری و اینترنت','ایران و فرهنگ','ادبیات و زبان','بازی و سرگرمی','خودرو و حمل‌ونقل','اختراعات و اکتشافات'].every(k=>bank[k]));
T('50 questions in existing categories',Object.entries(bank).filter(([k])=>['هنر و معماری','علم و طبیعت','ریاضی و منطق','ورزش','فیلم و سریال','موسیقی','اطلاعات عمومی'].includes(k)).every(([,levels])=>Object.values(levels).flat().length===50));
T('new categories have expanded banks',Object.entries(bank).filter(([k])=>['تاریخ','جغرافیا','غذا و خوراکی','حیوانات','فناوری و اینترنت','ایران و فرهنگ','ادبیات و زبان','بازی و سرگرمی','خودرو و حمل‌ونقل','اختراعات و اکتشافات'].includes(k)).every(([,levels])=>Object.values(levels).flat().length===20));
T('60 unique challenges',challenges.length===60&&new Set(challenges.map(x=>x.id)).size===60);
T('40 unique duels',duelTopics.length===40&&new Set(duelTopics.map(x=>x.id)).size===40);
T('duel contracts',duelTopics.every(d=>d.title&&d.topic&&d.rule&&Number.isFinite(d.seconds)));
T('no reused duel id',duelTopics.every((d,i)=>duelTopics.findIndex(x=>x.id===d.id)===i));
